...
class NetworkAnalyzer:
    ...
    def initialize_device(self, name=None):
        ...
        device = rm1.open_resource(na_name)
    ...
    def write_command(self, cmd_list):
        cmd = ';'.join(cmd_list)
        print('Sending command: ' + cmd)
        try:
    ...
    def print_old_trace(self, position=None, fig=None, ax=None):
        ...
        lim_cmd = 'OUTPLIML'
        str_res = self.send_command(na, [lim_cmd])
        if(str_res is None):
            return None, None
    ...
    def get_old_resp_complex(self):
        trace_format = 'FORM4'
        write_cmd = 'OUTPFORM'
        str_res = self.send_command([trace_format, write_cmd])
        real, imag = format_trace4_complex(str_res)
...
def format_trace4_complex(string_result):
    '''
    expected format: X.XXXXXXEXX, 0.000000E00\n
    '''
    lines = [x.strip() for x in string_result.split('\n')]
    print(lines)
    #real = [(line.split(',')[0].strip()) for line in lines]
    #print(real)
    split_data = [line.split(',') for line in lines]
    #imag = [(line.split(',')[1].strip()) for line in lines]
    #split_data = split_data.delete(-1)
    print(f'split data: {split_data}')
    #split_data = np.delete(split_data,-1)
    real = [i[0] for i in split_data[:-1]]
    imag = [i[1] for i in split_data[:-1]]
    print(split_data)
    print(real)
    print(imag)
    real_floats = []
    ...
    for p in imag:
        try: 
            imag_floats.append(float(p))
        except ValueError:
            #print('Not a float: ' + str(p))
            print('triggered2')
            pass
    
    print(f'real floats: {real_floats}')
    print(f'imag floats: {imag_floats}')
    return real_floats, imag_floats
